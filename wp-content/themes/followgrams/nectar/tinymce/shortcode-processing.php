...
function nectar_button($atts, $content = null) {  
    extract(shortcode_atts(array("label"=>"","placeholder"=>"","opus"=>"","hosted_button"=>"","size" => 'small', "url" => '#', 'color' => 'Accent-Color', "text" => 'Button Text', 'open_new_tab' => '0'), $atts));
	
	$target = ($open_new_tab == 'true') ? 'target="_blank"' : null;
	$button_open_tag='<form action="https://www.paypal.com/cgi-bin/webscr" method="post" target="_top" class="paypalform">
			<label style="font-size: 14px;">'.$label.'</label>
			<input type="hidden" name="page_style" value="Followgrams">
			<input type="hidden" name="cmd" value="_s-xclick">
			<input type="hidden" name="hosted_button_id" value="'.$hosted_button.'">
			<input type="hidden" name="no_shipping" value="1">
			<input type="hidden" name="lc" value="US">
			<input type="hidden" name="return" value="/success/">
			<input type="hidden" name="cancel_return" value="/cancel/">
			<input type="hidden" name="item_name" value="'.$opus.'">
			<input type="hidden" name="on0" value="'.$label.'">
			<input type="hidden" name="custom" value="" id="pap_dx8vc2s5">
			<div>
			<input type="text" name="os0" placeholder="'.$placeholder.'" style="width: 91%;margin-bottom: 19px;">
			</div>
			';
	
	switch ($size) {
		case 'small' :
			$button_open_tag1= '<button style="border:none;" class="nectar-button small '. strtolower($color) .'"'. $target;
			break;
		case 'medium' :
			$button_open_tag1= '<button style="border:none;" class="nectar-button medium ' . strtolower($color) .'"'. $target;
			break;
		case 'large' :
			$button_open_tag1 = '<button style="border:none;" class="nectar-button large '. strtolower($color) .'"'. $target;
			break;	
	}
    return $button_open_tag . $button_open_tag1.' href="' . $url . '">' . $text . '</button></form>';
}
...
function nectar_pricing_column($atts, $content = null) {
	...
	if($highlight == 'true') {
		$hightlight_reason_html = '<div class="hilit_block"><img src="/wp-content/themes/followgrams/img/Bestseller.png"/></div>';
		$highlight_class='hilllit_class';
	}
